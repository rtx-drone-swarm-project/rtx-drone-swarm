...
class VoronoiCoverage(BaseSearchAlgorithm):
    ...
    def initialize(self, mission: dict) -> None:
        ...
        mission["grid"] = build_dense_coverage_grid(mission["bounds"]).tolist()
    ...
    def get_target_waypoints(self, mission: dict, free_drones: List[dict]) -> Dict[str, Tuple[float, float]]:
        ...
        if "virtual_centroids" not in mission or len(mission["virtual_centroids"]) != k:
            seeds = _balanced_partition_seeds(mission["bounds"], k)
            virtual_np = seeds
            for _ in range(100):
        ...
        virtual_np = np.array(mission["virtual_centroids"])
